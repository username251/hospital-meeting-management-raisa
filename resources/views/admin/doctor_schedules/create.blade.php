    ...
    <section class="content">
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">Form Jadwal Dokter</h3>
            </div>
            <div class="card-body">
                <form action="{{ route('admin.doctor_schedules.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="form-group">
                        ...
                        <select name="is_available" id="is_available" class="form-control @error('is_available') is-invalid @enderror">
                            <option value="1" {{ old('is_available', true) == true ? 'selected' : '' }}>Ya</option>
                            <option value="0" {{ old('is_available', false) == false ? 'selected' : '' }}>Tidak</option>
                        </select>
                        @error('is_available')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
