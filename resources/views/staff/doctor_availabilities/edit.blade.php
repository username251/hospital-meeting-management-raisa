                ...
                <form action="{{ route('staff.doctor_availabilities.update', $doctorAvailability->id) }}" method="POST">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <select name="doctor_id" id="doctor_id" class="form-control @error('doctor_id') is-invalid @enderror">
                                @foreach ($doctors as $doctor)
                                    <option value="{{ $doctor->id }}" {{ old('doctor_id', $doctorAvailability->doctor_id) == $doctor->id ? 'selected' : '' }}>
                                        {{ $doctor->user->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select name="day_of_week" id="day_of_week" class="form-control @error('day_of_week') is-invalid @enderror">
                                @foreach ($daysOfWeek as $key => $day)
                                    <option value="{{ $key }}" {{ old('day_of_week', $doctorAvailability->day_of_week) == $key ? 'selected' : '' }}>
                                        {{ __($day) }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="time" name="start_time" id="start_time" class="form-control @error('start_time') is-invalid @enderror" value="{{ old('start_time', \Carbon\Carbon::parse($doctorAvailability->start_time)->format('H:i')) }}">
                            @error('start_time')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                        ...
                        <div class="form-group">
                            ...
                            <input type="time" name="end_time" id="end_time" class="form-control @error('end_time') is-invalid @enderror" value="{{ old('end_time', \Carbon\Carbon::parse($doctorAvailability->end_time)->format('H:i')) }}">
                            @error('end_time')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="slot_duration" id="slot_duration" class="form-control @error('slot_duration') is-invalid @enderror" value="{{ old('slot_duration', $doctorAvailability->slot_duration) }}" min="10">
                            @error('slot_duration')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
